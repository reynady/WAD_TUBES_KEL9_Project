...
<div class="container mx-auto p-6">
    ...
    <div class="bg-white p-8 rounded shadow-lg">
        @if($pengaduan->image)
            <img src="{{ asset('storage/' . $pengaduan->image) }}" alt="{{ $pengaduan->title }}" class="w-full h-96 object-cover rounded mb-4">
        @endif

        <div class="flex justify-between items-center mb-4">
            <h1 class="text-3xl font-bold text-gray-800">{{ $pengaduan->title }}</h1>
            <p class="text-sm text-gray-500 text-justify">{{ $pengaduan->created_at->timezone('Asia/Jakarta')->format('d M Y, H:i') }}</p>
        </div>
        <p class="mt-4 text-gray-700">{{ $pengaduan->content }}</p>
    </div>

    @if(auth()->check() && (auth()->user()->role == 'mahasiswa' || auth()->user()->role == 'admin'))
        <div class="mt-8">
            <h2 class="text-xl font-semibold mb-2">Tinggalkan Komentar</h2>
            <form method="POST" action="/pengaduan/{{ $pengaduan->id }}/comments">
                @csrf
